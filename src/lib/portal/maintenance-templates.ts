...
export type ChecklistItem = { key: string; group: string; label: string };
export type FunctionTest = { key: string; label: string };
export type Template = { model: MachineModel; checklist: ChecklistItem[]; functionTests: FunctionTest[] };
...
function checklist(rows: [string, string][]): ChecklistItem[] {
  return rows.map(([group, label], i) => ({ key: `c${i + 1}`, group, label }));
}
...
export const TEMPLATES: Record<MachineModel, Template> = {
  scrubmaster_b75r: {
    ...
    checklist: checklist([
      ["Battery", "Battery pack / charge condition"],
      ["Battery", "Terminals / connections / cables"],
      ["Battery", "On-board charger / charging cable"],
      ["Electrical", "Fuses / wiring / connectors"],
      ["Electrical", "Control panel / display / switches"],
      ["Brush", "Left brush"],
      ["Brush", "Right brush"],
  ...
  walk_behind_scrubber: {
    ...
    checklist: checklist([
      ...
      ["Electrical", "Battery / terminals / cables"],
      ["Electrical", "Battery charger / charging cable"],
      ["Electrical", "Fuses / wiring / connectors"],
